...
export const MUXINLAN_DIALOGUES = {
  ...
  phase1_entry: [
    ...
    { speaker: '茉莉(?)', text: '"让我猜猜...又是一个想挑战王者大人的蠢货吧？"', emotion: 'arrogant' as const },
    {
      speaker: '茉莉(?)',
  ...
  phase1_battle: [
    ...
    {
      speaker: '茉莉(?)',
      text: '"就这？就这点程度？杂鱼就是杂鱼呢~多练几年再来挑战本少爷吧"',
      emotion: 'arrogant' as const,
    },
    ...
    {
      speaker: '茉莉(?)',
      text: '"要不要哥哥我稍微放点水呀？啊，不用了，反正你也赢不了~"',
      emotion: 'arrogant' as const,
    },
    {
      speaker: '茉莉(?)',
      text: '"看你这么努力的样子，本少爷都有点不忍心了呢~骗你的啦♡"',
      emotion: 'arrogant' as const,
    },
    ...
    {
      speaker: '茉莉(?)',
      text: '"哎呀呀~小垃圾的体力快不行了吗？要不要哥哥我帮你一把呀~♡"',
      emotion: 'arrogant' as const,
    },
  ...
  phase2_battle: [
    ...
    { speaker: '꧁༺茉莉༻꧂', text: '"在王者面前...跪下吧，杂鱼。"', emotion: 'angry' as const },
    { speaker: '꧁༺茉莉༻꧂', text: '"这就是与王者为敌的下场。"', emotion: 'angry' as const },
    ...
    { speaker: '꧁༺茉莉༻꧂', text: '"这就是反抗王者的代价！好好享受吧！"', emotion: 'angry' as const },
  ...
  // 免疫束缚时的嘲笑（第一阶段）
  bind_immune_phase1: [
    { speaker: '茉莉(?)', text: '"哎呀呀~想用丝线束缚本少爷？你是不是搞错了什么？"', emotion: 'arrogant' as const },
  ...
  bind_immune_phase2: [
    ...
    { speaker: '꧁༺茉莉༻꧂', text: '"想困住王者？你配吗？"', emotion: 'angry' as const },
 ...
 * 初始化薇丝佩菈BOSS战
 */
export function initVesperaBoss(playerGender: string = '男'): void {
  bossState.isBossFight = true;
 ...
 * 获取薇丝佩菈随机战斗对话
 */
export function getVesperaRandomBattleDialogue(playerGender: string = '男'): BossDialogue | null {
  // 如果已使用自体献祭，使用献祭后对话
...
export function shouldUseVesperaSelfSacrifice(bossClimaxCount: number, playerGender: string): boolean {
  ...
  // 只有当BOSS高潮次数为2或3且玩家为男性时才使用
  // 注意：高潮次数2/3意味着高潮2次或3次后
  if ((bossClimaxCount === 2 || bossClimaxCount === 3) && playerGender !== '女') {
    return true;
  }
